...
(define-non-fungible-token ItemA uint)
...
(define-private (is-owner (actor principal) (token-id uint))
  (is-eq actor
       ...
       (unwrap! (nft-get-owner? ItemA token-id) false)))
...
(define-private (transfer-token (token-id uint) (owner principal) (new-owner principal))
  (let
    ...
    (begin
      ...
      (match (nft-transfer? ItemA token-id owner new-owner)
        success (ok success)
  ...
  (if (is-eq spender tx-sender)
      ...
      (if (or (is-owner tx-sender token-id)
              (is-operator-approved
               (unwrap! (nft-get-owner? ItemA token-id) nft-not-found-err)
               tx-sender))
          (begin
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? ItemA token-id)))
...
(define-read-only (get-token-uri (token-id uint))
  (ok (some "ItemA Contract Owner - Agent1"))
)
...
(begin
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u1))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u2))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u3))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u4))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u5))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u6))
  (try! (mint 'ST1SJ3DTE5DN7X54YDH5D64R3BCB6A2AG2ZQ8YPD5 u7))
)